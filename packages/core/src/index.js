...
exports.aksara = exports.AksaraConverter = void 0;
class AksaraConverter {
    constructor(options = { format: 'html' }) {
        this.options = {
            locale: 'id',
            pageSize: 'A4',
    ...
    async convertToPptx() {
        try {
            const PptxGenJS = (await Promise.resolve().then(() => __importStar(require('pptxgenjs')))).default;
     ...
     */
    async addPptxBackground(slide, backgroundPath) {
        try {
     ...
     */
    addPptxHeader(slide, pageNumber) {
        if (!this.directives.header)
            return;
     ...
     * Convert markdown content to PPTX elements
     */
    async addMarkdownToPptx(slide, markdown) {
     ...
     */
    async addPptxImage(slide, imageLine, defaultY) {
        try {
            const path = require('path');
            const fs = require('fs');
            // Extract image path
            ...
            if (attrs) {
                ...
                if (xMatch)
                    x = this.convertPercentToInches(xMatch[1], 10);
                if (yMatch)
                    y = this.convertPercentToInches(yMatch[1], 5.625);
                if (wMatch)
                    w = this.convertPercentToInches(wMatch[1], 10);
                if (hMatch && hMatch[1] !== 'auto')
                    h = this.convertPercentToInches(hMatch[1], 5.625);
            }
            // Read image as base64
            const imageData = fs.readFileSync(absolutePath);
            const base64Data = imageData.toString('base64');
     ...
     */
    convertPercentToInches(value, maxSize) {
        if (value.includes('%')) {
...
<body data-type="${isPresentation ? 'presentation' : 'document'}">
     ...
     */
    convertRelativeImagePaths(html) {
        const path = require('path');
        const fs = require('fs');
        // Function to convert a path to base64 data URL
        const convertPath = (imagePath) => {
...
<body data-type="${isPresentation ? 'presentation' : 'document'}">
     ...
     */
    getPresentationControls(totalSections) {
        return `
    <!-- Presentation Controls -->
     ...
     */
    getDocumentControls(totalSections) {
        return `
    <!-- Document Controls -->
...
exports.AksaraConverter = AksaraConverter;
// Export default instance
exports.aksara = new AksaraConverter();
